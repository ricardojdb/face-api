...
colors = [(150, 150, 150), (8, 154, 255), (0, 253, 255),
          (231, 217, 0), (0, 0, 190), (0, 184, 113), (98, 24, 91)]
colors = [tuple(reversed(i)) for i in colors]

classes = ['neutral', 'happiness', 'surprise',
...
app.layout = html.Div([
    html.Div([
        ...
        dcc.Interval(
            id='pie-chart-update', interval=1*1000,  n_intervals=0)],
            style={'float': 'left',
                   'width': '33%',
                   'text-align': 'right'},),
    ...
    html.Div([
        dcc.Graph(
            ...
            style={'display': 'inline-block', 'width': '100%'}),
        dcc.Interval(
            id='bar-chart-update', interval=1*1000, n_intervals=0)],
            style={'float': 'left',
                   'width': '33%',
                   'text-align': 'middle'},),
    ...
    html.Div([
        dcc.Graph(
            id='live-emotions-bar',
            style={'display': 'inline-block', 'width': '100%'}),
        dcc.Interval(
            id='emotions-update', interval=1*1000, n_intervals=0)],
            style={'float': 'left',
                   'width': '33%',
                   'text-align': 'left'},),
...
def update_graph_scatter(n):
    try:
        ...
        cursor = conn.cursor()
        df = pd.read_sql(
            "SELECT * FROM sentiment ORDER BY time DESC LIMIT 500", conn)
        ...
        conn.close()
        return {'data': data_list, 'layout': go.Layout(
...
def update_graph_pie(n):
    try:
        ...
        cursor = conn.cursor()
        df = pd.read_sql(
            "SELECT gender FROM sentiment ORDER BY time DESC LIMIT 1000", conn)
...
def update_graph_bar(n):
    try:
        ...
        df = pd.read_sql(
            "SELECT age FROM sentiment ORDER BY time DESC LIMIT 1000", conn)

        age_range = [[0, 18], [19, 25], [26, 30],
                     [31, 39], [40, 49], [50, 200]]

        values = np.array([
            sum(df.age.between(age[0], age[1])) for age in age_range])
...
def update_graph_main_emotions(n):
    try:
        ...
        df = pd.read_sql(
            "SELECT neutral, happiness, surprise, sadness, "
            " anger, disgust, fear, time FROM sentiment "
            "ORDER BY time DESC LIMIT 1000", conn)

        emotion_mean = df.mean()
        emotion_mean = round(emotion_mean*100)

        Labels = ['neutral', 'happiness', 'surprise
                  'sadness', 'anger', 'disgust', 'fear']
