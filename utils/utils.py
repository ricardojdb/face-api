...
colors = {
    'anger': (0, 0, 190), 'disgust': (0, 184, 113),
    'fear': (98, 24, 91), 'happiness': (8, 154, 255),
    'sadness': (231, 217, 0), 'surprise': (0, 253, 255),
...
def draw_box(image, label, gender, age, scores, classes, colors, box):
    ...
    for i, score in enumerate(scores):
        ...
        bar_text_size = cv2.getTextSize(
            bar_label, fontType, fontScale, thickness)
        # Emotion bars
        bar_x = xmax + 20
        ...
        if np.mean(gray[bar_ymin:bar_ymax, bar_xmin:bar_xmax]) > 100:
            font_color = (0, 0, 0)

        cv2.putText(img, bar_label, (bar_x, bar_y), fontType,
                    fontScale, font_color, thickness)
        cv2.line(img, (bar_x+5, bar_y + (bar_text_size[0][1])),
                 (bar_x+5+bar_w, bar_y + (bar_text_size[0][1])),
                 colors[classes[i]], int(fontScale*20))
...
def draw_box_emojis(image, label, gender, age, scores, classes, colors, box):
    ...
    name_size = cv2.getTextSize(
        label, cv2.FONT_HERSHEY_DUPLEX, text_name_scale, 1)

    # sex age size
    text_agen_scale = 0.6
    agen = str(age)+', ' + str(gender)
    agen_size = cv2.getTextSize(
        agen, cv2.FONT_HERSHEY_DUPLEX, text_agen_scale, 1)
    ...
    # infobox
    cv2.rectangle(
        image, (int(infobox_xmin), int(infobox_ymin)),
        (int(infobox_xmax), int(infobox_ymax)),
        color_box, -1)
    cv2.rectangle(
        image, (int(infobox_xmin), int(infobox_ymin)),
        (int(infobox_xmax), int(infobox_ymax)),
    ...
    cv2.putText(image, label, (ID_x, ID_y), cv2.FONT_HERSHEY_DUPLEX,
                text_name_scale, (255, 255, 255), 1)
    # write sex and age
    se_x = int(infobox_xmin+8)
    se_y = int(ID_y+5+agen_size[0][1])
    cv2.putText(image, agen, (se_x, se_y), cv2.FONT_HERSHEY_DUPLEX,
                text_agen_scale, (255, 255, 255), 1)
    ...
    true_h, true_w, _ = image[emoji_ymin:emoji_ymin+emo_h,
                              emoji_xmin:emoji_xmin+emo_w].shape

    image[emoji_ymin:emoji_ymin+emo_h,
          emoji_xmin:emoji_xmin+emo_w] *= np.expand_dims(
                                                mask[:true_h, :true_w], -1)
    image[emoji_ymin:emoji_ymin+emo_h,
          emoji_xmin:emoji_xmin+emo_w] += emoji[:true_h, :true_w]
            ...
            try:
                ...
                if self.data_list is not None:
                    for data in self.data_list:
                        scores_dic = {
                            c: s*100 for c, s in zip(classes, data[3:11])}

                        img = draw_box_emojis(
                            img, data[0], data[1], int(data[2]),
