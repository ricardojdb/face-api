...
def create_mysql_table():
    ...
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS sentiment(name TEXT, gender TEXT, age REAL,"
...
def insert_mysql_table(data_list):
    ...
    for data in data_list:
        data = data[:11] + [data[-1]]
        c.execute("INSERT INTO sentiment (name, gender, age,"
                  "neutral, happiness, surprise, sadness, anger, "
                  "disgust, fear, contempt, time) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", [str(x) for x in data])
        conn.commit()
...
def create_mysql_ranktable():
    ...
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS ranktable(name TEXT, mainsentiment TEXT, score REAL, photo TEXT)")
    conn.commit()
